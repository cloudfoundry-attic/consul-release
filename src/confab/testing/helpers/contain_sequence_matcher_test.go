...
var _ = Describe("ContainSequence", func() {
	Context("when actual is not a slice", func() {
		It("should error", func() {
			_, err := helpers.ContainSequence(func() {}).Match("not a slice")
		...
		Context("when the entire sequence is present", func() {
			It("should succeed", func() {
				Expect([]string{
					...
					"value-3",
					"value-4",
				}).To(helpers.ContainSequence(sequence))
			})
		})
		...
		Context("when some of the sequence is present", func() {
			It("should fail", func() {
				Expect([]string{
					...
					"value-3",
					"value-4",
				}).NotTo(helpers.ContainSequence(sequence))
			})
		})
	...
	Describe("FailureMessage", func() {
		It("returns an understandable error message", func() {
			Expect(helpers.ContainSequence([]int{1, 2, 3}).FailureMessage([]int{5, 6})).To(Equal("Expected\n    <[]int | len:2, cap:2>: [5, 6]\nto contain sequence\n    <[]int | len:3, cap:3>: [1, 2, 3]"))
		})
	})

	Describe("NegatedFailureMessage", func() {
		It("returns an understandable error message", func() {
			Expect(helpers.ContainSequence([]int{1, 2, 3}).NegatedFailureMessage([]int{1, 2, 3})).To(Equal("Expected\n    <[]int | len:3, cap:3>: [1, 2, 3]\nnot to contain sequence\n    <[]int | len:3, cap:3>: [1, 2, 3]"))
		})
	})
})
