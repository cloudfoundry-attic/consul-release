...
var _ = Describe("recursor timeout", func() {
	...
	BeforeEach(func() {
		By("deploying turbulence", func() {
			...
			turbulenceClient = turbulenceclient.NewClient(fmt.Sprintf("https://turbulence:%s@%s:8080", turbulencePassword.(string), turbulenceIPs[0]), 10*time.Minute, 2*time.Second)
		})
