...
import (
	...
	"github.com/greenhouse-org/consul-release/src/acceptance-tests/testing/consulclient"
...
var _ = Describe("Scaling up Instances", func() {
	...
	Describe("scaling from 1 node to 3", func() {
		BeforeEach(func() {
			...
			manifest, err = helpers.DeployConsulWithOpsWithInstanceCount("scale-up-1-to-3", 1, boshClient)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("scaling from 3 nodes to 5", func() {
		BeforeEach(func() {
			...
			manifest, err = helpers.DeployConsulWithOpsWithInstanceCount("scale-up-3-to-5", 3, boshClient)
			Expect(err).NotTo(HaveOccurred())
